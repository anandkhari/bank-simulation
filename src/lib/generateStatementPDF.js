...
const styles = StyleSheet.create({
  ...
  logoBox: { width: 36, height: 42, marginRight: 6 },
  bankName: { fontSize: 7, fontFamily: "Helvetica-Bold", lineHeight: 1.4 },
  bankAddress: { fontSize: 7, lineHeight: 1.4, color: "#333333" },
  refLine: {
    fontSize: 7,
    color: "#888888",
    fontFamily: "Courier",
  ...
  accountNumberLabel: { fontSize: 9, fontFamily: "Times-Bold" },
  accountNumberValue: { fontSize: 9, fontFamily: "Times-Bold", letterSpacing: 1 },
  dividerBlack: {
    borderBottomWidth: 1,
    borderBottomColor: "#000000",
    marginVertical: 4,
  },
  reachUsTitle: { fontSize: 9, fontFamily: "Times-Bold", marginBottom: 2 },
  reachUsText: {
    fontSize: 8,
    fontFamily: "Times-Roman",
    lineHeight: 1.4,
  ...
  summaryTitle: { fontSize: 13, fontFamily: "Times-Bold", marginBottom: 6 },
  summaryAccountType: { fontSize: 8, fontFamily: "Helvetica-Bold", marginBottom: 2 },
  summaryBranchName: { fontSize: 9, fontFamily: "Helvetica-Bold", lineHeight: 1.4 },
  ...
  summaryValueBold: { fontSize: 8, fontFamily: "Helvetica-Bold", textAlign: "right" },
  activityTitle: { fontSize: 13, fontFamily: "Helvetica-Bold", marginBottom: 8 },
  tableHeaderRow: {
    flexDirection: "row",
    borderBottomWidth: 2,
    borderBottomColor: "#000000",
  ...
  tableHeaderCell: { fontSize: 8, fontFamily: "Helvetica-Bold" },
  openingRow: {
    flexDirection: "row",
    borderBottomWidth: 1,
    borderBottomColor: "#000000",
...
function RBCLogo() {
  return (
    <Svg viewBox="0 0 39.346668 45.666668" style={styles.logoBox}>
      ...
      <Path d="M34.477 39.696c0 1.476-.606 2.333-1.787 2.787-4.592 1.77-10.03 2.684-15.202 2.684-5.172 0-10.609-.913-15.202-2.684C1.106 42.029.5 41.173.5 39.696V.504h33.977z" fill="none" stroke="#fff" strokeWidth=".9999750000000001" />
...
function StatementDocument({ account, statement, transactions }) {
  return (
    <Document title={`Business Account Statement - ${account.account_number || ""}`}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.headerRow}>
            ...
            <View style={styles.headerLeft}>
              <View style={styles.logoRow}>
                ...
                <View>
                  ...
                  <Text style={styles.bankAddress}>P.O. BOX 4047 TERMINAL A</Text>
                  <Text style={styles.bankAddress}>TORONTO ON  M5W 1L5</Text>
                </View>
              </View>
              <Text style={styles.refLine}>RBBDA30000_4780138 E  D  03282     00101</Text>
            ...
            <View style={styles.headerRight}>
              <Text style={styles.statementTitle}>Business Account Statement</Text>
              ...
              <View style={styles.accountNumberRow}>
                ...
                <Text style={styles.accountNumberValue}>{account.account_number || ""}</Text>
              </View>
              <View style={styles.dividerBlack} />
              <Text style={styles.reachUsTitle}>How to reach us:</Text>
              <Text style={styles.reachUsText}>Please contact your RBC Banking representative or call</Text>
              <Text style={styles.reachUsText}>1-800-Royal®2-0</Text>
              <Text style={styles.reachUsText}>(1-800-769-2520)</Text>
              <Text style={styles.reachUsText}>www.rbcroyalbank.com/business</Text>
              <View style={styles.dividerBlack} />
            </View>
          </View>
          ...
          <View style={styles.summaryContainer}>
            <Text style={styles.summaryTitle}>Account Summary for this Period</Text>
            <Text style={styles.summaryAccountType}>Royal Business Account ®</Text>
            <Text style={styles.summaryBranchName}>Royal Bank of Canada</Text>
            <Text style={styles.summaryBranchAddress}>29 HURON ST, NEW HAMBURG, ON N3A 1K1</Text>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Opening Balance on {formatDate(statement.start_date)}</Text>
              <Text style={styles.summaryValue}>{formatMoney(statement.opening_bal)}</Text>
            </View>
            <View style={styles.summaryRow}>
              <Text style={styles.summaryLabel}>Total deposits & credits ({statement.total_deposit_count ?? ""})</Text>
              <Text style={styles.summaryValue}>+ {formatMoney(statement.total_deposits)}</Text>
            </View>
            <View style={styles.summaryRowThick}>
              <Text style={styles.summaryLabel}>Total cheques & debits ({statement.total_debit_count ?? ""})</Text>
              <Text style={styles.summaryValue}>- {formatMoney(Math.abs(statement.total_debits ?? 0))}</Text>
            </View>
            <View style={styles.summaryRowLast}>
              <Text style={styles.summaryLabelBold}>Closing balance on {formatDate(statement.end_date)}</Text>
              <Text style={styles.summaryValueBold}>= {formatMoney(statement.closing_bal)}</Text>
            </View>
          </View>
          ...
          <View style={styles.tableHeaderRow}>
            ...
            <Text style={[styles.tableHeaderCell, styles.colDebit]}>Cheques & Debits($)</Text>
            <Text style={[styles.tableHeaderCell, styles.colCredit]}>Deposits & Credits($)</Text>
          ...
          <View style={styles.openingRow}>
            ...
            <Text style={[styles.cellTextBold, styles.colBalance]}>{formatMoney(statement.opening_bal)}</Text>
          </View>
          ...
          {transactions.map((t, i) => {
            ...
            return (
              <View key={i} style={styles.tableRow}>
                <Text style={[styles.cellText, styles.colDate]}>{i === 0 || transactions[i - 1]?.date !== t.date ? formatShortDate(t.date) : ""}</Text>
                <Text style={[styles.cellText, styles.colDesc]}>{t.description ?? ""}</Text>
                <Text style={[styles.cellText, styles.colDebit]}>{(t.debit ?? 0) > 0 ? formatMoney(t.debit) : ""}</Text>
                <Text style={[styles.cellText, styles.colCredit]}>{(t.credit ?? 0) > 0 ? formatMoney(t.credit) : ""}</Text>
                <Text style={[styles.cellText, styles.colBalance]}>{formatMoney(t.balance_after)}</Text>
              </View>
            );
          })}
...
export async function generateStatementPDF({ account, statement, transactions }) {
  ...
  if (!Array.isArray(transactions) || transactions.length === 0)
    throw new Error("generateStatementPDF: transactions must be a non-empty array");
